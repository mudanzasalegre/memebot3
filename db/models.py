...
class Token(Base):
    ...
    #  **NUEVOS nombres canónicos + default=0.0**
    liquidity_usd:   Mapped[float] = mapped_column(
        Float, default=0.0, server_default="0"
    )
    volume_24h_usd:  Mapped[float] = mapped_column(
        Float, default=0.0, server_default="0"
    )
...
class Position(Base):
    ...
    opened_at:  Mapped[_dt.datetime] = mapped_column(DateTime, default=_dt.datetime.utcnow)
...
class RevivedToken(Base):
    ...
    token_address: Mapped[str] = mapped_column(String, ForeignKey("tokens.address"), primary_key=True)
    first_listed:  Mapped[_dt.datetime] = mapped_column(DateTime)
    revived_at:    Mapped[_dt.datetime] = mapped_column(DateTime)
