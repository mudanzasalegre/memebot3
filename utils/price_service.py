...
TTL de errores configurable desde .env (DEXS_TTL_NIL).
"""
...
_TTL_ERR  = int(os.getenv("DEXS_TTL_NIL", "15"))  # viene de .env
_CHAIN    = "solana"
...
async def get_price(address: str, *, use_gt: bool = False) -> Optional[Dict[str, Any]]:
    ...
    if tok and not _needs_fallback(tok):
        logger.debug("[price_service] Precio obtenido desde DexScreener para %s", address[:4])
    ...
    if _USE_BIRDEYE:
        try:
            ...
            be = None
        if be:
            logger.debug("[price_service] Fallback → Birdeye para %s", address[:4])
    ...
    if use_gt and USE_GECKO_TERMINAL:
        ...
        if gt:
            logger.debug("[price_service] Fallback → GeckoTerminal para %s", address[:4])
    ...
    if tok and not _is_missing(tok.get("price_usd")):
        logger.debug("[price_service] Fallback → Conversión price_native×SOL para %s", address[:4])
        cache_set(ck, tok, ttl=_TTL_OK)
        return tok
