...
class _Config:
    ...
    MIN_MARKET_CAP_USD: float = _num_env(
        "MIN_MARKET_CAP_USD", float, 5_000
    )  # ★
    MAX_MARKET_CAP_USD: float = _num_env(
        "MAX_MARKET_CAP_USD", float, 20_000
    )  # ★
...
DB_URI = (
    f"sqlite+aiosqlite:///{pathlib.Path(SQLITE_DB).expanduser().resolve()}"
)
