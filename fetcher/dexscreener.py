...
2025-07-20  ·  Revisión
──────────────────────
• Liquidez y volumen ahora se guardan como **np.nan** cuando la API no los
  trae en los primeros minutos, evitando columnas “muertas” (todo 0).
• Se añaden varios fall-backs: liqLockedUsd, liq_locked / price, etc.
"""
...
def _extract_liquidity_usd(raw: dict, price_usd: float | None) -> float | None:
    """Intenta varios campos hasta obtener la liquidez en USD."""
...
def _norm(raw: dict) -> dict:
    ...
    tok = {
        ...
        # se pasa todo el raw para que `sanitize_token_data` haga su magia
        **raw,
    }
...
async def get_pair(address: str) -> Optional[Dict]:
    """
    Devuelve un dict normalizado con liquidez/volumen **o** None si DexScreener
    aún no tiene constancia del par.
    Se cachea (TTL):
      • 2 min si ok
      • 1 h si sin datos (“NIL”)
    """
    ck = f"dex:{address}"
    hit = cache_get(ck)
    if hit is not None:
