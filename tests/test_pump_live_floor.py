...
def _load_quality_namespace(**overrides: object):
    ...
    namespace = {
        ...
        "_PUMP_EARLY_PROFIT_BLOCK_MCAP_MIN_USD": 25_000.0,
        "_PUMP_EARLY_PROFIT_BLOCK_MCAP_MAX_USD": 50_000.0,
        "_PUMP_EARLY_PROFIT_BLOCK_PRICE5M_RANGES": ((0.0, 25.0), (50.0, 100.0)),
        ...
        "_PUMP_EARLY_PROFIT_MAX_MARKET_CAP_USD": 500_000.0,
...
def test_pumpswap_profit_gate_blocks_toxic_mcap_and_price5m_buckets() -> None:
    gate = _load_entry_quality_gate(_PUMP_EARLY_SNIPER_ENABLED=True, _PUMP_EARLY_PROFIT_LANE_ENABLED=True)
    token = {
        "dex_id": "pumpswap",
        "age_min": 6.0,
    ...
    assert ok is False
    assert "mcap_block:25000_50000" in reason
    assert "price5m_50_100" in reason
    assert token["entry_lane"] == "pump_early_sniper_research"
...
def test_pumpswap_profit_shape_guard_blocks_extreme_spike_high_mcap() -> None:
    ...
    assert ok is False
    assert "shape_mcap>=500000" in reason
